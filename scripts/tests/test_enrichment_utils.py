...
class TestDiscoverEnrichments(unittest.TestCase):
    ...
    def test_discover_enrichments_non_existent_directory(self):
        ...
        enrichment_classes = enrichment_utils.discover_enrichments(["/path/that/does/not/exist"], self.logger)
    ...
    def test_discover_enrichments_file_instead_of_directory(self):
        ...
        enrichment_classes = enrichment_utils.discover_enrichments([str(test_file)], self.logger)
...
""")
            ...
            enrichment_classes = enrichment_utils.discover_enrichments([str(self.test_dir), tmpdir], self.logger)
    ...
    def test_discover_enrichments_ignores_dunder_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            enrichment_classes = enrichment_utils.discover_enrichments([tmpdir], self.logger)
...
class TestParseEnrichmentArgs(unittest.TestCase):
    ...
    def test_parse_valid_argument(self):
        ...
        self.assertEqual(result, {"SimpleEnrichment": {"param1": "value1"}})

    def test_parse_multiple_arguments_same_enrichment(self):
        """Test parsing multiple arguments for the same enrichment."""
        args = ["ParametricEnrichment:param1=value1", "ParametricEnrichment:param2=value2"]
        result = enrichment_utils.parse_enrichment_args(args, self.logger)

        self.assertEqual(result, {"ParametricEnrichment": {"param1": "value1", "param2": "value2"}})

    def test_parse_arguments_multiple_enrichments(self):
        """Test parsing arguments for different enrichments."""
        args = ["EnrichmentA:param1=valueA", "EnrichmentB:param1=valueB"]
        result = enrichment_utils.parse_enrichment_args(args, self.logger)

        self.assertEqual(result, {"EnrichmentA": {"param1": "valueA"}, "EnrichmentB": {"param1": "valueB"}})
    ...
    def test_parse_argument_with_special_characters(self):
        ...
        self.assertEqual(result, {"Enrichment": {"url": "https://example.com/path?key=value"}})
...
class TestInstantiateEnrichments(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.enrichment_classes = enrichment_utils.discover_enrichments([str(self.test_dir)], self.logger)
...
class TestAddEnrichmentArguments(unittest.TestCase):
    ...
    def test_add_enrichment_arguments_allows_chaining(self):
        ...
        args = parser.parse_args(["in.txt", "--enrichment-dir", "/path", "out.txt"])
