...
class TestSARIFConverterMethods:
    ...
    @pytest.mark.parametrize("scenario,run,expected_metadata", [
        (
        ...
        (
            'missing_name_defaults_to_unknown',
            {'tool': {'driver': {}}},
            {'name': 'Unknown'}
        ),
    ])
    ...
    def test_extract_finding_info_basic(self, converter):
        ...
        result = {
            'ruleId': 'CWE-457',
            'message': {
                'text': 'Using uninitialized value'
            }
        }
            ...
            'tool': {
                'driver': {
                    'rules': [
                        {
                            'id': 'RULE-001',
                            'shortDescription': {'text': 'Test rule 1'}
                        },
                        {
                            'id': 'RULE-002',
                            'shortDescription': {'text': 'Test rule 2'}
                        }
                    ]
                }
            }
...
class TestSARIFIntegration(unittest.TestCase):
    ...
    def test_save_to_file(self):
        ...
        try:
            ...
            self.assertEqual(loaded_findings[0]['finding_info']['title'],
                           findings[0]['finding_info']['title'])
        finally:
            # Clean up
