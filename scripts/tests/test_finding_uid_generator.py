...
def test_hash_based_uid_generation(sast_enrichment):
    ...
    finding = {
        ...
        "vulnerabilities": [{"affected_code": {"file": "src/api/query.py", "start_line": 42, "end_line": 45}}],
    }
...
def test_fingerprint_selection_alphabetical_order(sast_enrichment):
    ...
    finding = {
        "metadata": {"product": {"name": "TestTool"}},
        "finding_info": {"uid": "PLACEHOLDER_UID", "title": "TEST-001", "desc": "Test finding"},
        "enrichments": [
            {
                "name": "fingerprints",
                "data": {"csdiff/v0": "hash0", "csdiff/v1": "hash1", "csdiff/v2": "hash2"},
            }
        ],
    }
    ...
    expected_hash = hashlib.sha256(b"hash2").hexdigest()
...
def test_tool_name_normalization(sast_enrichment):
    ...
    finding = {
        "metadata": {"product": {"name": "My Super Tool"}},
        "finding_info": {"uid": "PLACEHOLDER_UID", "title": "TEST-001", "desc": "Test finding"},
    }
...
def test_tool_name_normalization_with_special_chars(sast_enrichment):
    ...
    for input_name, expected_normalized in test_cases:
        finding = {
            "metadata": {"product": {"name": input_name}},
            "finding_info": {"uid": "PLACEHOLDER_UID", "title": "TEST-001", "desc": "Test finding"},
        }
...
def test_custom_uid_generator():
    ...
    enrichment = FindingUIDGenerator(sdlc_type="pentest", uid_generator=custom_generator)

    finding = {"jira_key": "RHEL-12345", "finding_info": {"uid": "PLACEHOLDER_UID"}}
...
def test_sdlc_type_normalization():
    ...
    for input_sdlc, expected_normalized in test_cases:
        ...
        finding = {"finding_info": {"uid": "PLACEHOLDER_UID"}}
...
def test_boann_prefix_always_used(sast_enrichment):
    ...
    finding = {
        "metadata": {"product": {"name": "SonarQube"}},
        "finding_info": {"uid": "PLACEHOLDER_UID", "title": "SONAR-001", "desc": "Test finding"},
    }
