...
class FindingUIDGenerator(EnrichmentPlugin):
    ...
    def __init__(self, sdlc_type: str = "sast", uid_generator: Callable | None = None):
        """
    ...
    def enrich(self, finding: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to generate UID: {e}", exc_info=logger.isEnabledFor(logging.DEBUG))
    ...
    def _try_fingerprint_approach(self, finding: dict[str, Any]) -> str | None:
        """
    ...
    def _hash_based_approach(self, finding: dict[str, Any]) -> str:
        ...
        logger.debug(
            f"Generated hash from components: {[c[:30] + '...' if len(c) > 30 else c for c in hash_components]}"
        )
    ...
    def _add_uid_generation_metadata(self, finding: dict[str, Any], method: str, version: str) -> None:
        """
