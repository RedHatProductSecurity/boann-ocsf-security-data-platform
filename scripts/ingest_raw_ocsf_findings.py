...
- Each row corresponds to a single OCSF finding event from a specific scan
- JSONB column (raw_ocsf_json) stores the individual finding's JSON payload
- No unique constraints - same finding_uid can appear multiple times
- Uses append-only INSERT strategy to preserve all scan data
...
class OCSFIngestor:
    ...
    def store_findings_to_db(self, ocsf_events_list: list, source_filename: str) -> int:
        ...
        Uses append-only INSERT strategy - same finding_uid can appear multiple times to preserve
        all scan data for scan-level tracking.
        ...
        for event in ocsf_events_list:
            ...
            try:
                stmt = insert(self.raw_ocsf_findings_table).values(
                ...
                with self.db_engine.begin() as conn:
                    conn.execute(stmt)
                total_loaded += 1
                self.logger.debug("Inserted finding %s", finding_id)
            except Exception as e:
                self.logger.error("Failed to insert finding %s: %s", finding_id, e, exc_info=True)
                raise
